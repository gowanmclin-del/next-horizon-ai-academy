...
export default function AssessmentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!configured) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-0">
        <DevBanner>
          this environment has no Supabase connection, so the assessment can&rsquo;t be loaded or graded here.
        </DevBanner>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="container-page py-16 text-center">
        ...
        <a href={`/login?redirect=/courses/${course.slug}/assessment`} className="mt-4 inline-block font-semibold text-horizon-blue">
          Log In
        </a>
      </div>
    );
  ...
  if (!enrolled) {
    return (
      <div className="container-page py-16 text-center">
        <p className="text-slate-600">You need to be enrolled in {course.title} to take this assessment.</p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-0">
      <DevBanner>
        your score is calculated and recorded inside the database — this page never computes or trusts a score from
        the browser.
      </DevBanner>
